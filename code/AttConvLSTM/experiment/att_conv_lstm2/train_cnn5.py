...
#
#         # conv temp
#         conv_temp = ld.conv_layer(conv2, 3, 1, 8, "encode_temp",is_bn=CONF['IS_BN'])
#         if not isTest:
#             conv_temp=tf.layers.dropout(conv_temp,rate=CONF['KEEP_PROB'])
#
#         # conv3
#         conv3 = ld.conv_layer(conv_temp, 3, 2, 8, "encode_3",is_bn=CONF['IS_BN'])
#         if not isTest:
#             conv3=tf.layers.dropout(conv3,rate=CONF['KEEP_PROB'])
#         # conv4
#         conv4 = ld.conv_layer(conv3, 1, 1, 4, "encode_4",is_bn=CONF['IS_BN'])
...
#
#     # conv7
#     conv_test = ld.transpose_conv_layer(conv6, 3, 1, 8, "decode_test", is_bn=CONF['IS_BN'])
#     if not isTest:
#         conv_test = tf.layers.dropout(conv_test, rate=CONF['K/home/linc/Desktop/nm_ddpg/mobile_terminal_envEEP_PROB'])
#
#     # conv7
#     conv7 = ld.transpose_conv_layer(conv_test, 3, 1, 8, "decode_7",is_bn=CONF['IS_BN'])
#     if not isTest:
#         conv7 = tf.layers.dropout(conv7, rate=CONF['KEEP_PROB'])
#     # x_1
#     x_1 = ld.transpose_conv_layer(conv7, 3, 2, 2, "decode_8",linear=True,is_bn=False)  # set activation to linear
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        for i in xrange(CONF['SEQ_LENGTH']):
            if i < CONF['SEQ_START']:
        ...
        loss = CONF['BETA'] * rmse_loss + mape_loss
        # rmse_loss = tf.reduce_mean(tf.squared_difference(x[:, CONF['SEQ_START']:, :, :, :] * env.scale,
        ...
        min_rmse = 100000
        min_mape = 100000
        for step in xrange(CONF['STEP_START'], CONF['MAX_STEP']):
            ...
            if step % 1000 == 0:
                ...
                print("mean rmse is ", mean_rmse, 'mean mape is ', mean_mape, "mean alpha is", np.mean(a_l_g_list,
                                                                                                       axis=0),
                      "min_rmse", min_rmse, 'min_mape', min_mape, )
